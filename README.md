...
### What we learned
What worked well for this pen plotter was obtaining the hardware we needed and creating models that could be easily 3D printed and assembled. We noticed other groups struggled obtaining and purchasing materials, while most of our parts could be printed or found in the scrap bin. We were also able to complete tests to see how our motors moved the system around to ensure the hardware functioned as intended in the design. However, from this project we learned the importance of testing our code early and often. We made the mistake of believing the software portion of our project wouldn't be too complicated as we planned to reuse several files from older projects. We ran into some difficulty getting all the commands from the HPGL file to read as sometimes commands would get skipped over. We eventually learned this was likely due to either calling queue.read() twice as well as cotask running tasks either too quick or slow. There were other errors that we were unable to fix and from this we learned to not underestimate how long the software portion of a project will take. We also learned the importance of designing for as much resolution as possible in the hardware design, so controlling the motors in software would be much easier. Our rotational motor only had 1/4 revolution of a turn in resolution, which made positional control for complex shapes practically impossible. Sticking to a direct drive shaft attached to a smaller wheel would have been a much smarter choice as shown in the original example. 
...
In the future we would recommend investigating interpolating the HPGL commands so there isn't as much noise. We would also recommend using direct drive for spinning a small wheel, therefore there is more resolution for position control. Finally, implementing finite states machines for each task early on would improve overall code readability and allow for quick code diagnosing. 
